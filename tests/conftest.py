...
# Add project paths needed by tests regardless of how pytest is invoked.
...
ConfigManager = importlib.import_module("shared.config").ConfigManager
...
def mock_drives(monkeypatch):
    ...
    mock_drive_list = [
        DriveInfo(
            letter="C:", total_gb=500.0, free_gb=100.0, used_gb=400.0, percent_used=80.0
        ),
        DriveInfo(
            letter="D:", total_gb=1000.0, free_gb=5.0, used_gb=995.0, percent_used=99.5
        ),  # Low space
    ]

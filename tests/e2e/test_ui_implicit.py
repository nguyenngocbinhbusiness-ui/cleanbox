...
class TestUI018TrayIcon:
    def test_tc_ui_018_n(self, qapp):
        assert TrayIcon() is not None

    def test_tc_ui_018_e(self, qapp):
        assert load_icon() is not None

    def test_tc_ui_018_ed(self, qapp):
...
class TestUI019TrayCleanupNow:
    def test_tc_ui_019_n(self, qapp):
        c = []
        t = TrayIcon(on_cleanup=lambda: c.append(1))
        t._handle_cleanup(None, None)
        assert len(c) == 1
    ...
    def test_tc_ui_019_ed(self, qapp):
        TrayIcon()._handle_cleanup(None, None)


class TestUI020TraySettings:
    def test_tc_ui_020_n(self, qapp):
        c = []
        t = TrayIcon(on_settings=lambda: c.append(1))
    ...
    def test_tc_ui_020_ed(self, qapp):
        assert True
...
class TestUI021TrayExit:
    def test_tc_ui_021_n(self, qapp):
        c = []
        t = TrayIcon(on_exit=lambda: c.append(1))
        t._handle_exit(None, None)
        assert len(c) == 1

    def test_tc_ui_021_e(self, qapp):
        TrayIcon()._handle_exit(None, None)

    def test_tc_ui_021_ed(self, qapp):
        t = TrayIcon()
        t.stop()
...
class TestUI022CloseButton:
    def test_tc_ui_022_n(self, qapp):
        w = MainWindow()
        w.show()
        w.close()
        assert not w.isVisible()

    def test_tc_ui_022_e(self, qapp):
        w = MainWindow()
        w.show()
        w.close()
        assert not w.isVisible()
...
class TestUI023FileDialog:
    def test_tc_ui_023_n(self, qapp):
        assert hasattr(CleanupView(), "_on_add_directory")

    def test_tc_ui_023_e(self, qapp):
        v = CleanupView()
...
class TestUI024AlreadyAddedMessage:
    def test_tc_ui_024_n(self, qapp):
        assert True

    def test_tc_ui_024_e(self, qapp):
        assert True

    def test_tc_ui_024_ed(self, qapp):
        assert True
...
class TestUI025NoSelectionMessage:
    def test_tc_ui_025_n(self, qapp, monkeypatch):
        m = []
        monkeypatch.setattr(QMessageBox, "information", lambda *a: m.append(a))
        CleanupView()._on_remove_directory()
        assert len(m) == 1

    def test_tc_ui_025_e(self, qapp):
        assert True

    def test_tc_ui_025_ed(self, qapp):
        assert True
...
class TestIMPSEC001InputValidation:
    def test_tc_imp_sec_001_n(self, fresh_config):
        fresh_config.add_directory("C:\\Valid\\Path")
        assert True

    def test_tc_imp_sec_001_e(self, fresh_config):
        fresh_config.add_directory("")
        assert True  # Should not crash

    def test_tc_imp_sec_001_ed(self, fresh_config):
        fresh_config.add_directory("C:\\тест\\フォルダ")
        assert True


class TestIMPSEC002PathTraversal:
    def test_tc_imp_sec_002_n(self):
        s = CleanupService()
        s.cleanup_directory("C:\\Normal")
    ...
    def test_tc_imp_sec_002_ed(self):
        assert True
...
class TestIMPACC001KeyboardNav:
    def test_tc_imp_acc_001_n(self, qapp):
        w = MainWindow()
        for b in w.sidebar.buttons.values():
            assert b.isEnabled()
    ...
    def test_tc_imp_acc_001_ed(self, qapp):
        assert True
...
class TestIMPACC002Contrast:
    def test_tc_imp_acc_002_n(self, qapp):
        w = MainWindow()
        assert "#1A1A1A" in w.styleSheet() or "color:" in w.styleSheet()

    def test_tc_imp_acc_002_e(self, qapp):
        assert True

    def test_tc_imp_acc_002_ed(self, qapp):
        assert True
...
class TestIMPPERF001UIResponse:
    ...
    def test_tc_imp_perf_001_ed(self, qapp):
        ...
        v = StorageView()
        assert v._drive_combo is not None
...
class TestIMPPERF002NonBlocking:
    ...
    def test_tc_imp_perf_002_ed(self, qapp):
        assert True
...
class TestIMPUX001ErrorMessages:
    def test_tc_imp_ux_001_n(self):
        s = CleanupService()
        r = s.cleanup_directory("C:\\NonExistent12345")
        assert len(r.errors) >= 1

    def test_tc_imp_ux_001_e(self):
        s = CleanupService()
        r = s.cleanup_directory("???")
        assert isinstance(r.errors, list)

    def test_tc_imp_ux_001_ed(self):
        assert True
...
class TestIMPUX002LoadingStates:
    ...
    def test_tc_imp_ux_002_e(self, qapp):
        assert True

    def test_tc_imp_ux_002_ed(self, qapp):
        assert True
...
class TestIMPUX003Confirmation:
    def test_tc_imp_ux_003_n(self, qapp):
        v = CleanupView()
        v._on_cleanup()  # Should work without confirmation

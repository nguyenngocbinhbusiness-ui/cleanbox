"""Storage tree item construction engine."""
...
class TreeItemRowData:
    """Structured folder row payload before Qt item assembly."""
...
def _get_generic_folder_icon() -> QIcon:
    ...
    if _GENERIC_FOLDER_ICON is None:
        _GENERIC_FOLDER_ICON = _icon_provider.icon(QFileIconProvider.IconType.Folder)
...
def _get_generic_file_icon() -> QIcon:
    ...
    if _GENERIC_FILE_ICON is None:
        _GENERIC_FILE_ICON = _icon_provider.icon(QFileIconProvider.IconType.File)
...
def build_tree_row_data(
    folder_info: FolderInfo,
    reference_size: int,
    is_root: bool = False,
) -> TreeItemRowData:
    """Build a structured row payload for folder tree rendering."""
    row = build_folder_row_values(folder_info, reference_size)
    return TreeItemRowData(
        name_text=row["name_text"],
        size_text=row["size_text"],
        allocated_text=row["allocated_text"],
        files_text=row["files_text"],
        folders_text=row["folders_text"],
...
def build_tree_item(
    ...
    try:
        ...
        row = build_tree_row_data(folder_info, reference_size, is_root=is_root)
        ...
        item.setData(COL_NAME, ROLE_IS_ROOT, row.is_root)
        ...
        logger.error("Failed to build tree item: %s", error)
...
def add_file_entries(
    ...
    try:
        ...
        if summary.omitted_count > 0:
            ...
            more_item.setText(
                COL_NAME,
                f"{format_size(summary.omitted_size)}   [{summary.omitted_count:,} more files...]",
            )
        ...
        logger.error("Failed to add file entries: %s", error)
